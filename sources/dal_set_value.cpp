...
dal_t&	dal_t::operator=(uint32_t value)
{
	//Delete our child elements
	dal_t*	child		= this->_child;
	while (child != NULL)
...
dal_t&	dal_t::operator=(dalProxy_t value)
{
	//Delete our child elements
	dal_t*	child		= this->_child;
	while (child != NULL)
	...
	//Delete allocated memory for non-trivial types
	if (this->_type & ALLOCATED_MEM_TYPE)	_dal_memHooks.free_data(this->_mem_ptr);
	if (value.node == nullptr)	return *this;

	this->_child		= value.node->_child;
	value.node->_child	= nullptr;
	this->_last			= value.node->_last;
	...
	_copy_trivial(value.node);

	dal_delete(value.node);
