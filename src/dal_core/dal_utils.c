...
dalMemHooks_t			__dal_mem_hooks		= {NULL};
...
{
	dal_t*	newObj		= (dal_t*)__dal_mem_hooks.alloc_node();
...
dal_t*	dal_create_array(const char* key)
{
	...
	{
		newObj->key_len		= (uint8_t)strlen(key) + 1;	//+1 takes into account the 0 at the end of the line
...
void	dal_change_key(dal_t* obj, const char* newKey)
{
	...
	obj->key_len			= (uint8_t)strlen(newKey);
...
dalResult_e	_dal_delete_recurse(dal_t* item)
{
	switch (item->type)
	{
		...
		case DT_UINT:
		case DT_INT:
		case DT_DOUBLE:		__dal_mem_hooks.free_node(item);
							break;
		case DT_STRING:		__dal_mem_hooks.free_data(item->mem_ptr);
							__dal_mem_hooks.free_node(item);
							break;
		case DT_BLOB:		__dal_mem_hooks.free_data(item->mem_ptr);
							__dal_mem_hooks.free_node(item);
							break;
		case DT_BLOB_REF:	__dal_mem_hooks.free_node(item);			break;
		case DT_ARRAY:
...
dal_t*	dal_add_obj(dal_t* obj, const char* key)
{
	...
	if ((key != NULL) && (*key != 0x00))
	{
		newObj->key_len	= (uint8_t)strlen(key) + 1;	//+1 учитывает 0 в конце строки
		newObj->key_len	= (newObj->key_len > sizeof(newObj->key)) ? sizeof(newObj->key) : newObj->key_len;
		memcpy(newObj->key, key, newObj->key_len-1);
	}
...
dal_t*	dal_add_cstr(dal_t* dst, const char* key, const char* value)
{
	...
	newObj->mem_ptr			= __dal_mem_hooks.alloc_data(valLen + 8);
	newObj->asSTRING		= (char*)((size_t)((uint8_t*)newObj->mem_ptr + 7) & ~(size_t)0x07);
...
dal_t*	dal_add_str(dal_t* dst, const char* key, char* value, uint32_t len)
{
	...
	newObj->mem_ptr			= __dal_mem_hooks.alloc_data(len + 8);
	newObj->asSTRING		= (char*)((size_t)((uint8_t*)newObj->mem_ptr + 7) & ~(size_t)0x07);
	if (newObj->asSTRING == NULL)
	{	dal_delete(newObj);
		return NULL;
...
dal_t*	dal_add_blob(dal_t* dst, const char* key, void** value, uint32_t len)
{
	*value					= NULL;
	...
	newObj->type			= DT_BLOB;
	newObj->mem_ptr			= __dal_mem_hooks.alloc_data(len + 8);
	newObj->asBLOB			= (uint8_t*)((size_t)((uint8_t*)newObj->mem_ptr + 7) & ~(size_t)0x07);
